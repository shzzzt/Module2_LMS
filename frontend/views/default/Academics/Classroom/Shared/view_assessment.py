...
class ViewAssessment(QWidget):
    ...
    def __init__(self, assessment_data, user_role, controller, parent=None):
        super().__init__(parent)
        self.assessment_data = assessment_data
        self.user_role = user_role
        self.controller = controller  # Use shared controller
        
        # Extract post_id from assessment_data if available
        self.post_id = assessment_data.get('post_id')
    ...
    def populate_data(self):
        ...
        if self.score_label:
            score = self.assessment_data.get("score", "10")
            self.score_label.setText(f"{score} points")
    ...
    def show_menu(self):
        ...
        if self.user_role in ["faculty", "admin"]:
            ...
            edit_action.triggered.connect(self.edit_assessment)
            delete_action.triggered.connect(self.delete_assessment)
    ...
    def edit_assessment(self):
        """Handle edit assessment using controller"""
        if self.post_id:
            print(f"Editing assessment with ID: {self.post_id}")
            # This would open an edit dialog, for now just placeholder
            updated_data = {
                'title': f"Updated {self.assessment_data.get('title', 'Assessment')}",
    ...
    def delete_assessment(self):
        """Handle delete assessment using controller"""
        if self.post_id:
            print(f"Deleting assessment with ID: {self.post_id}")
    ...
    def on_error_occurred(self, error_message):
        ...
        print(f"View Assessment Error: {error_message}")
...
if __name__ == "__main__":
    ...
    view_assessment = ViewAssessment(assessment_data, "faculty", controller)
    view_assessment.setFixedSize(940, 530)
